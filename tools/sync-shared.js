...
//
// The five games share their plumbing - the input mask, the touch controls, the
// three-letter name entry, the speech synthesiser and the maths - and share it
// by having the same file in every repository rather than by a package, because
// none of them has a build step and none is going to get one for this.
//
...
const NEIGHBOURS = [
  ...
  ['webtype', process.env.WEBTYPE || path.resolve(HERE, '..', 'webtype')],
];

/**
 * What is generic enough to be identical in all five games.
 *
 ...
 * with new pictures:
 *
 *   src/highscores.js   football keeps a scoreline, the shooter keeps a run that
 *                       only goes up, and this keeps a time - where smaller
 *                       wins, so every comparison runs the other way.
 *   src/net/*           there is no multiplayer here, so there is no transport,
 *                       no signalling and no relay. That is not a gap; it is the
 *                       one design decision this game made that the others did
 *                       not.
 *   worker/index.js     the same, minus the room half: a board and nothing else.
 *
 ...
 * pretending otherwise would mean a shared file full of `if (motorcycles)`.
 */
export const SHARED = [
  'src/util.js',
  'src/input.js',
...
if (process.argv[1] && process.argv[1].endsWith('sync-shared.js')) {
  ...
  console.log('OK: all five games are running the same plumbing underneath');
}
